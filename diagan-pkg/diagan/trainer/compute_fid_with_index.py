...
from diagan.datasets.image_loader_with_index import get_dataset_images_with_index
from torch_mimicry.metrics.fid import fid_utils
from torch_mimicry.metrics.inception_model import inception_utils


def compute_real_dist_stats_with_index(index,
                                       sess,
                                       batch_size,
                                       dataset=None,
                                       stats_file=None,
    ...
    if stats_file is None:
        ...
        stats_file = os.path.join(
            stats_dir,
            "fid_stats_{}_drs_{}_{}_run_{}.npz".format(name, dataset, len(index),
                                                 seed))
    ...
    if stats_file and os.path.exists(stats_file):
        ...
        images = get_dataset_images_with_index(dataset, index=index)
...
def fid_score_with_index(index,
                         num_fake_samples,
                         netG,
                         dataset,
                         seed=0,
    ...
    Args:
        index (array): The index array of real images to use for FID.
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        m_real, s_real = compute_real_dist_stats_with_index(index=index,
                                                            sess=sess,
                                                            dataset=dataset,
