...
from diagan.datasets.image_loader import get_dataset_images
from diagan.trainer import fid_utils
...
def compute_real_dist_stats(num_samples,
    ...
    if stats_file and os.path.exists(stats_file):
        ...
        images = get_dataset_images(dataset, data_path=data_path, num_samples=num_samples)
...
def compute_gen_dist_stats(netG,
    ...
    gen_images_file = os.path.join(
            save_dir,
            "fid_gen_images_{}k_{}.npy".format(num_samples // 1000,
                                                 seed))
    if not os.path.exists(gen_images_file):
        with torch.no_grad():
            ...
            print("INFO: Saving generated images...")
            np.save(gen_images_file, images)
    else:
        print("INFO: Loading generated images...")
        images = np.load(gen_images_file)
