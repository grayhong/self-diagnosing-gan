...
def main():
    ...
    set_seed(opt.seed)
    print(opt)
    model = SimpleConvNet(num_channels = 1, num_labels = 20)
    ckpt_path = Path(f'./exp_results/mnist-fmnist-convnet-60000-seed1')
    ...
    #num_fake = round(opt.num_data*(1-opt.major_ratio))
    num_fake = 10000
    if opt.drs:
    ...
    num_minor = sum(ds_train.dataset.mixed_targets == 1)
    print("num_minor: ", num_minor)

    major_dict = compute_pr(real_feats[ds_train.dataset.mixed_targets == 0][:num_minor].cpu().numpy(), fake_feats.cpu().numpy(), nearest_k=3, device='cuda')
    minor_dict = compute_pr(real_feats[ds_train.dataset.mixed_targets == 1].cpu().numpy(), fake_feats.cpu().numpy(), nearest_k=3, device='cuda')

    print(f"Major: {major_dict}")
    print(f"Minor: {minor_dict}")

    save_path = f'{opt.work_dir}/mnist_fmnist_prdc'
    ...
    dict_dict = {'Major': major_dict, 'Minor': minor_dict }
