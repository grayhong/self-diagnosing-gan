...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, output_path):
    ...
    for idx in pbar:
        ...
        if i % args.logit_save_steps == 0 and i >= args.save_logit_after and i <= args.stop_save_logit_after:
            print(f'save logit step: {i}')
            logit_list = get_logit(dataloader=loader, netD=discriminator, device=device)
            if get_rank() == 0:
                logit_results['netD'][i] = logit_list
                save_logit(logit_results, output_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--r1", type=float, default=0.1, help="weight of the r1 regularization"
    )
    parser.add_argument(
        "--path_regularize",
        type=float,
    ...
    parser.add_argument('--save_logit_after', default=195000, type=int)
    parser.add_argument('--stop_save_logit_after', default=200000, type=int)
    ...
    train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, save_path)
